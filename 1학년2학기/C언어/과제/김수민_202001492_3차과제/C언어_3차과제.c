...
int main(void) {
	...
	//구조체 동적 할당
	Student *s_list = (Student*)malloc(N_student*sizeof(Student));
	if (s_list == NULL) exit(1);
	for (int i = 0; i < N_student; i++) {
		s_list[i].sub = (float*)malloc(N_sub*sizeof(float));
		if (s_list[i].sub == NULL) exit(1);
	}
	//과목 이름 동적 할당
	char ** sub_name = (char**)malloc(N_sub * sizeof(char*));
	if (sub_name == NULL) exit(1);
	for (int i = 0; i < N_sub; i++) {
		sub_name[i] = (char*)malloc(10 * sizeof(char));
		if (sub_name[i] == NULL) exit(1);
	}
